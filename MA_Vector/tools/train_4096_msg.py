...
sys.path.append("..")
from model.Model_2_msg import Model
...
def train():
    vector_training_data_path = '../training_set_preprocess_4096/'
    csv_file_path = '../result_vector_with_0_50%.csv'
    vector_test_data_path ='../test_set_preprocess_4096/'
...
#*****************Step1 Setup Data***********************
#load trainingset
    train_set = TrainingSets(csv_file_path,vector_training_data_path,4096) 
    train_loader = torch.utils.data.DataLoader(train_set,batch_size=14,shuffle=True,num_workers=2)
#load testset
    test_set = TestSets(vector_test_data_path,4096)
...
# set tensor-board for visualization
    writer = SummaryWriter('runs_4096_msg')
...
# training
    ...
    for epoch in range(EPOCH):
        ...
        scheduler.step()
    torch.save(model.state_dict(), "model_4096_msg.pkl")
